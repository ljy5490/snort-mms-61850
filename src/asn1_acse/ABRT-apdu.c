/*
 * Generated by asn1c-0.9.29 (http://lionet.info/asn1c)
 * From ASN.1 module "ACSE-1"
 * 	found in "ACSE.asn1"
 * 	`asn1c -fcompound-names`
 */

#include "ABRT-apdu.h"

asn_TYPE_member_t asn_MBR_ABRT_apdu_1[] = {
	{ ATF_NOFLAGS, 0, offsetof(struct ABRT_apdu, abort_source),
		(ASN_TAG_CLASS_CONTEXT | (0 << 2)),
		-1,	/* IMPLICIT tag at current level */
		&asn_DEF_ABRT_source,
		0,
		{ 0, 0, 0 },
		0, 0, /* No default value */
		"abort-source"
		},
	{ ATF_POINTER, 2, offsetof(struct ABRT_apdu, abort_diagnostic),
		(ASN_TAG_CLASS_CONTEXT | (1 << 2)),
		-1,	/* IMPLICIT tag at current level */
		&asn_DEF_ABRT_diagnostic,
		0,
		{ 0, 0, 0 },
		0, 0, /* No default value */
		"abort-diagnostic"
		},
	{ ATF_POINTER, 1, offsetof(struct ABRT_apdu, user_information),
		(ASN_TAG_CLASS_CONTEXT | (30 << 2)),
		-1,	/* IMPLICIT tag at current level */
		&asn_DEF_Association_information,
		0,
		{ 0, 0, 0 },
		0, 0, /* No default value */
		"user-information"
		},
};
static const int asn_MAP_ABRT_apdu_oms_1[] = { 1, 2 };
static const ber_tlv_tag_t asn_DEF_ABRT_apdu_tags_1[] = {
	(ASN_TAG_CLASS_APPLICATION | (4 << 2)),
	(ASN_TAG_CLASS_UNIVERSAL | (16 << 2))
};
static const asn_TYPE_tag2member_t asn_MAP_ABRT_apdu_tag2el_1[] = {
    { (ASN_TAG_CLASS_CONTEXT | (0 << 2)), 0, 0, 0 }, /* abort-source */
    { (ASN_TAG_CLASS_CONTEXT | (1 << 2)), 1, 0, 0 }, /* abort-diagnostic */
    { (ASN_TAG_CLASS_CONTEXT | (30 << 2)), 2, 0, 0 } /* user-information */
};
asn_SEQUENCE_specifics_t asn_SPC_ABRT_apdu_specs_1 = {
	sizeof(struct ABRT_apdu),
	offsetof(struct ABRT_apdu, _asn_ctx),
	asn_MAP_ABRT_apdu_tag2el_1,
	3,	/* Count of tags in the map */
	asn_MAP_ABRT_apdu_oms_1,	/* Optional members */
	2, 0,	/* Root/Additions */
	3,	/* First extension addition */
};
asn_TYPE_descriptor_t asn_DEF_ABRT_apdu = {
	"ABRT-apdu",
	"ABRT-apdu",
	&asn_OP_SEQUENCE,
	asn_DEF_ABRT_apdu_tags_1,
	sizeof(asn_DEF_ABRT_apdu_tags_1)
		/sizeof(asn_DEF_ABRT_apdu_tags_1[0]) - 1, /* 1 */
	asn_DEF_ABRT_apdu_tags_1,	/* Same as above */
	sizeof(asn_DEF_ABRT_apdu_tags_1)
		/sizeof(asn_DEF_ABRT_apdu_tags_1[0]), /* 2 */
	{ 0, 0, SEQUENCE_constraint },
	asn_MBR_ABRT_apdu_1,
	3,	/* Elements count */
	&asn_SPC_ABRT_apdu_specs_1	/* Additional specs */
};

